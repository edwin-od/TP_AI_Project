        ...
        public override void OnStart()
        {
            ...
            for (int i = 0; i < hyperion.currentData.WayPoints.Count; i++)
            {
                if (hyperion.currentData.WayPoints[i].Owner != hyperion.mySpaceship.Owner)
                {
                    ...
                    Debug.DrawLine(spaceship.Position, spaceship.Velocity, Color.red);
                    Debug.DrawLine(spaceship.Position, new Vector2(Mathf.Cos(angleOffset), Mathf.Sin(angleOffset)) * Mathf.Rad2Deg, Color.yellow);
                    Debug.DrawLine(spaceship.Position, new Vector2(Mathf.Cos(-angleOffset), Mathf.Sin(-angleOffset)) * Mathf.Rad2Deg, Color.yellow);
